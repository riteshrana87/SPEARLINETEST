...
class MonthlyReportController extends Controller
{
	...
	*/
    public function index(Request $request){
        ...
        /* Start Js data */
		$data['page_js'] = array(
            'backend/assets/monthlyReport/js/monthly_report.js'
        );
        /* End Js data */
        /* Start CSS data */
        ...
        if ($request->ajax()) {
           ...
           if($request->start && $request->end){
             $from = date('Y-m-d',strtotime($request->start.'-01'));
             $to = date('Y-m-t',strtotime($request->end.'-10'));
        ...
        return view('monthlyReport.monthly_report_list',$data);
    }
}
